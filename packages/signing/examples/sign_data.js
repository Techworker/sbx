...
const DataOperation = require('@pascalcoin-sbx/signing').Operations.Data;
...
rpc.getAccount(1430880)
  ...
  .then((account) => {
    const op = new DataOperation(1430880, 1430880, 1430881);

    op.withPayload(BC.fromString('test'));
    op.withMinFee();
    op.withDataType(1);
    op.withDataSequence(2);
    op.withAmount(0.0001);
    op.sign(keyPair, account.nOperation + 1);

    console.log(DataOperation.fromRaw(op.toRaw()));
