...
describe('Core.Types.Account', () => {
  it('can be created from raw and contains valid values', () => {
    ...
    accounts.forEach((raw) => {
      let account = Account.createFromObject(raw);

      expect(account.accountNumber.account).to.be.equal(raw.account);
      ...
      expect(account.balance.toStringOpt()).to.be.equal(raw.balance_s.toString());

      expect(account.nOperation).to.be.equal(raw.n_operation);
      expect(account.updatedInBlockNumber).to.be.equal(raw.updated_b);
      ...
      if (raw.locked_until_block === undefined) {
        expect(account.lockedUntilBlockNumber).to.be.equal(null);
      } else {
        expect(account.lockedUntilBlockNumber).to.be.equal(raw.locked_until_block);
      }
      ...
      if (raw.seller_account === undefined) {
        expect(account.sellerAccountNumber).to.be.equal(null);
      } else {
        expect(account.sellerAccountNumber).to.be.instanceof(AccountNumber);
        expect(account.sellerAccountNumber.account).to.be.equal(raw.seller_account);
      }

      if (raw.private_sale === undefined) {
        expect(account.isPrivateSale).to.be.equal(false);
      } else {
        expect(account.isPrivateSale).to.be.equal(raw.private_sale);
      }
      ...
      if (raw.receiver_swap_account === undefined) {
        expect(account.receiverSwapAccountNumber).to.be.equal(null);
      } else {
        expect(account.receiverSwapAccountNumber).to.be.instanceof(AccountNumber);
        expect(account.receiverSwapAccountNumber.account).to.be.equal(raw.receiver_swap_account);
      }
    });
